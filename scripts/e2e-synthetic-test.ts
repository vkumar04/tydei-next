...
async function tieInAndCollectionDateCheck(
  ...
  // the tydei compute layer, compare against the reference math.
...
/* ───────────── bundle compute: DB round-trip vs reference math ───────────── */
...
async function bundleRoundTripCheck(
  ...
  const { tieInAllOrNothing, crossVendorTieIn } = await import(
    "@/lib/contracts/tie-in-bundle-math"
  )
  ...
  if (
    ...
    !approx(computed1.allOrNothing.rebateEarned, ref1.rebateEarned) ||
    computed1.allOrNothing.bonusLevel !== ref1.bonusLevel
  ) {
    failures.push({
      where: "bundle round-trip AON base",
      detail: `v0 $${ref1.rebateEarned}/${ref1.bonusLevel} vs tydei ${JSON.stringify(computed1?.allOrNothing)}`,
    })
  }
  ...
  if (
    ...
    !approx(computed3.crossVendor.totalRebate, ref3.totalRebate) ||
    computed3.crossVendor.allCompliant !== ref3.allCompliant
  ) {
    failures.push({
      where: "bundle round-trip cross_vendor",
      detail: `v0 $${ref3.totalRebate} vs tydei ${JSON.stringify(computed3?.crossVendor)}`,
    })
  }
...
async function main(): Promise<number> {
  ...
  try {
    ...
    // in tydei, not in the reference. See `lib/contracts/tie-in-bundle-math.ts` and
