...
async function regenerateOne(contract: ContractRow) {
  ...
  // Charles W1.V — scale `rebateValue` at the Prisma boundary so the
  // engine receives integer percent (3) rather than the stored fraction
  // (0.03). Mirrors `recomputeAccrualForContract` / `getAccrualTimeline`.
