...
import { type PeriodRow } from "@/lib/contracts/transactions-display"
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        {/* Re-mount the form per selected row via `key`, so its useState
            fields initialize fresh from `row` instead of being re-seeded
            by a reset effect. Guarded on `row` so the inner component can
            assume a non-null row. */}
        {row && (
          <EditTransactionForm
            key={row.id}
...
function EditTransactionForm({
  ...
  const [amount, setAmount] = useState(() => String(row.rebateCollected ?? 0))
  const [earnedAmount, setEarnedAmount] = useState(() =>
    String(row.rebateEarned ?? 0),
  )
  const [date, setDate] = useState(() =>
    row.collectionDate ? row.collectionDate.slice(0, 10) : "",
  )
