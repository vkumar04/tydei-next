...
import type { VendorReportsContract } from "./vendor-reports-types"

/**
 * Vendor Calculations tab — mirror of the facility
 * `ReportsCalculationsTab`. Full rebate calculation audit trail for a
 * vendor's contract: contract info, tier structure, formula, included +
 * excluded POs, and a tier-progress projection. Data from
 * `getVendorRebateCalculationAudit`.
 */
export interface VendorReportsCalculationsTabProps {
  vendorId: string
...
export function VendorReportsCalculationsTab({
  vendorId,
  contracts,
  selectedContract,
  onSelectContract,
}: VendorReportsCalculationsTabProps) {
  if (!selectedContract) {
    return (
      <Card>
        <CardHeader>
...
function CalculationsForContract({
  ...
  const { data, isLoading } = useQuery({
    queryKey: queryKeys.vendorReports.audit(vendorId, contractId),
    queryFn: () => getVendorRebateCalculationAudit(contractId),
  })
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <FormulaBlock label="Formula" text={data.calc.formula} />
          <FormulaBlock label="Detailed" text={data.calc.detailedFormula} />
        </CardContent>
      </Card>
