...
| Vendor 1-/2-way mode (the ONE mode-gated read: facility actuals flow to the vendor ONLY when an `accepted` connection exists AND `mode === two_way`) | `getFacilityActualsForVendor` (`lib/actions/vendor-prospective.ts`) + `setConnectionMode` / `get/setVendorOperatingMode` (vendor-only, `lib/actions/connection-mode.ts`) | `lib/actions/vendor-prospective.ts` + `lib/actions/connection-mode.ts` | mode lives on `Connection.mode` (per facility–vendor pair); `Vendor.defaultMode` is the STANDALONE vendor's own operating mode, never a facility-data grant; 1-way vendors self-serve contracts + may enter own COG. The facility-side contract-flow gate `vendorContractsVisibleToFacility` was REMOVED as dead code 2026-07-03 (never enforced; Vick "remove them if they're not being used") — re-derive from the actuals gate's where if ever wanted. Regression-guarded by `facility-actuals-for-vendor.test.ts` + `connection-mode.test.ts`. |
...
- **Two-way sync auto-pull:** `getFacilityActualsForVendor(facilityId,
  ...
  over synced values (fill-only-when-empty). The never-enforced facility-side
  gate `vendorContractsVisibleToFacility` (and the unused `getConnectionMode`
  read) were REMOVED 2026-07-03 — the actuals gate is the one mode gate.
