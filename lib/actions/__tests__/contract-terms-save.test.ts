...
vi.mock("@/lib/db", () => ({
  prisma: {
    contract: {
      // Charles audit round-9 BLOCKER: contract-terms actions now
      // verify ownership via prisma.contract.findUniqueOrThrow before
      // mutating. Mock returns the contractId so the check passes.
