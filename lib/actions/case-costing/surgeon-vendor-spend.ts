...
export async function getSurgeonVendorSpend(
  ...
  // ─── 2. normalized SKU → vendor name (off-contract supplies) ─────
  // From the facility's COG rows — this is the "pull from COGS" linkage.
  const cog = await prisma.cOGRecord.findMany({
    where: { facilityId: facility.id, vendorName: { not: null } },
    select: { vendorItemNo: true, inventoryNumber: true, vendorName: true },
  })
  ...
  for (const r of cog) {
    ...
    for (const sku of [r.vendorItemNo, r.inventoryNumber]) {
      ...
      if (n && !vendorNameBySku.has(n)) vendorNameBySku.set(n, r.vendorName)
    }
  }
