 ...
 * never reimplement). `recomputeVolumeFromCogRecords` below calls these
 * same functions, so writer behavior is unchanged and the two surfaces
 * cannot drift. Persisted `[auto-volume-accrual]` rows are still written
 * ONLY for closed windows (W1.W-B1 honesty rule — ledger earned stays
 * `payPeriodEnd <= today`).
 */
...
async function recomputeVolumeFromCogRecords(input: {
  ...
  // moved to the exported pure helpers above so the accrual timeline's
  // in-progress display calls the SAME functions — writer-consistent by
  // construction, zero behavior change here.
